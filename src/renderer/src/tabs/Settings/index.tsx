...
function PathList({ label, paths, onChange, placeholder, hint }: { label: string; paths: string[]; onChange: (p: string[]) => void; placeholder?: string; hint?: string }): React.ReactElement {
  const [newPath, setNewPath] = useState('')
  return (
    <div className="flex flex-col gap-2">
      <label className="text-sm font-medium" style={{ color: 'var(--color-text)' }}>{label}</label>
      {hint && (
        <p className="text-xs" style={{ color: 'var(--color-text-dim)', fontFamily: 'var(--font-mono)' }}>↪ auto-scans: {hint}</p>
      )}
      {paths.map((p, i) => (
...
export const SettingsTab: React.FC = () => {
  ...
  const [xdgDefaults, setXdgDefaults] = useState<{ videosDir: string; musicDir: string } | null>(null)

  useEffect(() => {
    window.htpc.app.getXdgDefaults().then(setXdgDefaults).catch(() => {})
  }, [])
    ...
    <div className="h-full overflow-y-auto gpu-scroll">
      <div className="max-w-2xl mx-auto p-6 flex flex-col gap-8">
        ...
        <section className="flex flex-col gap-4">
          ...
          <PathList label="Movie Paths" paths={settings.moviePaths} onChange={(p) => update({ moviePaths: p })} placeholder={xdgDefaults?.videosDir} hint={xdgDefaults?.videosDir} />
          <PathList label="Music Paths" paths={settings.musicPaths} onChange={(p) => update({ musicPaths: p })} placeholder={xdgDefaults?.musicDir} hint={xdgDefaults?.musicDir} />

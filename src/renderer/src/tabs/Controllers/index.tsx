...
import { ControllerDevice, ControllerType, ButtonMapping, NormalizedInputEvent } from '../../../../shared/types'
...
const BUTTON_LABELS: Record<string, string> = {
  ...
  left_bumper: 'LB / L1', right_bumper: 'RB / R1',
  left_trigger: 'LT / L2', right_trigger: 'RT / R2',
  select: 'Back / Share', start: 'Start / Options', home: 'Guide / PS',
  left_thumb: 'LS / L3', right_thumb: 'RS / R3',
  touchpad: 'Touchpad',
...
const MappingRow: React.FC<MappingRowProps> = ({
  inputCode, currentAction, isLearning, learnedRaw,
  onStartLearn, onCancelLearn, onSave, onHover
}) => {
  const [editing, setEditing] = useState(false)
  const [selected, setSelected] = useState(currentAction)

  useEffect(() => { setSelected(currentAction) }, [currentAction])

  return (
    <motion.div
      className="flex items-center justify-between py-2 px-3 rounded gap-2"
      style={{
        background: isLearning ? 'color-mix(in srgb, var(--color-accent) 18%, var(--color-surface-raised))' : 'var(--color-surface-raised)',
    ...
    >
      <span className="text-sm font-mono w-28 shrink-0" style={{ color: 'var(--color-text)' }}>
      ...
      {isLearning ? (
          ...
          <div className="flex gap-2">
            {learnedRaw !== null && (
              <motion.button
                ...
                onClick={() => { onSave(currentAction || DEFAULT_ACTIONS[0]); onCancelLearn() }}
                whileTap={{ scale: 0.96 }}
              >
                Save
              </motion.button>
        ...
        <div className="flex flex-1 items-center gap-2 justify-between">
          ...
          >
            {DEFAULT_ACTIONS.map((a) => <option key={a} value={a}>{a}</option>)}
          </select>
        ...
        <div className="flex flex-1 items-center gap-2 justify-between">
          <span className="text-sm" style={{ color: currentAction ? 'var(--color-accent)' : 'var(--color-text-dim)' }}>
            {currentAction || '—'}
          </span>
          <div className="flex gap-1">
            <motion.button
...
function DiagramForDevice({ type, highlight, learn }: { type: ControllerType; highlight: string | null; learn: string | null }) {
  if (type === 'xbox') return <XboxController highlightCode={highlight} learnCode={learn} />
  if (type === 'ps4' || type === 'ps5' || type === 'ps3') return <PS4Controller highlightCode={highlight} learnCode={learn} />
  return null
}
...
export const ControllersTab: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsub = window.htpc.input.onEvent((ev: NormalizedInputEvent) => {
      if (ev.type === 'button_press' && learningRef.current && ev.rawCode !== undefined) {
  ...
  const saveMapping = useCallback((inputCode: string, action: string, rawCode?: number | null): void => {
    if (!selectedDevice) return
    const codeToSave = rawCode !== null && rawCode !== undefined ? String(rawCode) : inputCode
    window.htpc.input.setMapping(selectedDevice.id, codeToSave, action)
    setMappings((prev) => {
      const idx = prev.findIndex((m) => m.inputCode === codeToSave)
      const entry: ButtonMapping = { deviceId: selectedDevice.id, inputCode: codeToSave, action }
  ...
  const hasDiagram = selectedDevice && (
  ...
  return (
    <div className="h-full flex gap-4 p-4 overflow-hidden">
      ...
      <div className="w-60 flex-shrink-0 flex flex-col gap-3">
        <h2 className="text-base font-semibold" style={{ color: 'var(--color-text)' }}>Devices</h2>

        {devices.length === 0 ? (
          <div className="text-sm text-center py-8" style={{ color: 'var(--color-text-dim)' }}>
            No controllers detected.<br />
            <span className="text-xs opacity-60">User must be in the <code>input</code> group.</span>
          </div>
        ) : (
        ...
        {lastEvent && (
          <div className="mt-auto p-3 rounded-[var(--radius-card)] text-xs" style={{ background: 'var(--color-surface-raised)', border: '1px solid var(--color-border)' }}>
            ...
            <div style={{ color: 'var(--color-text-dim)' }}>
              ...
              {lastEvent.rawCode !== undefined && <div>Raw: {lastEvent.rawCode}</div>}
      ...
      <div className="flex-1 overflow-y-auto gpu-scroll">
        {selectedDevice ? (
          <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm capitalize" style={{ color: 'var(--color-text-dim)' }}>
                    {selectedDevice.type} · {selectedDevice.buttonCount}b · {selectedDevice.axisCount}ax
                    {selectedDevice.vendorId ? ` · ${selectedDevice.vendorId.toString(16).padStart(4,'0')}:${selectedDevice.productId?.toString(16).padStart(4,'0')}` : ''}
                  </p>
                </div>
              </div>
              {/* Reset to default */}
              <AnimatePresence mode="wait">
                {confirmReset ? (
                  <motion.div
                    ...
                    exit={{ opacity: 0, x: 8 }}
                  >
                    <span className="text-xs" style={{ color: 'var(--color-text-dim)' }}>Clear all mappings?</span>
                    <motion.button
                      className="px-3 py-1 rounded text-xs"
            ...
            {/* SVG diagram */}
            {hasDiagram && (
              <div className="rounded-[var(--radius-card)] p-3" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
                <DiagramForDevice
            ...
            <AnimatePresence>
              {learningCode && (
                <motion.div
                  ...
                  style={{ background: 'color-mix(in srgb, var(--color-accent) 22%, var(--color-surface))', border: '1px solid var(--color-accent)' }}
                ...
                >
                  ...
                  <span style={{ color: 'var(--color-text)' }}>
                    Learning <strong>{BUTTON_LABELS[learningCode] ?? learningCode}</strong> — press any button on the controller
                    {learnedRaw !== null && <span style={{ color: 'var(--color-accent)' }}> · raw code: {learnedRaw}</span>}
                  </span>
                  <motion.button
            ...
            {/* Mappings list */}
            <h3 className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--color-text-dim)' }}>Button Mappings</h3>
              ...
              {BUTTON_CODES.map((code) => {
                ...
                return (
                  <MappingRow
                    ...
                    onSave={(action) => saveMapping(code, action, learningCode === code ? learnedRaw : null)}

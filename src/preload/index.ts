...
const htpc = {
  settings: {
    ...
    set: (partial: Partial<AppSettings>): Promise<void> => ipcRenderer.invoke('settings:set', partial)
  },
  ...
  games: {
    scan: (extraPaths?: string[]): Promise<Game[]> => ipcRenderer.invoke('games:scan', extraPaths),
    list: (): Promise<Game[]> => ipcRenderer.invoke('games:list'),
    launch: (game: Game): Promise<void> => ipcRenderer.invoke('games:launch', game),
    favorite: (id: string, value: boolean): Promise<void> => ipcRenderer.invoke('games:favorite', id, value),
    tag: (id: string, tags: string[]): Promise<void> => ipcRenderer.invoke('games:tag', id, tags),
    fetchMetadata: (title: string, steamAppId?: number): Promise<unknown> =>
      ipcRenderer.invoke('games:metadata', title, steamAppId)
  },

  movies: {
    scan: (extraPaths?: string[]): Promise<Movie[]> => ipcRenderer.invoke('movies:scan', extraPaths),
  ...
  tv: {
    scan: (extraPaths?: string[]): Promise<TVShow[]> => ipcRenderer.invoke('tv:scan', extraPaths),
  ...
  input: {
    devices: (): Promise<ControllerDevice[]> => ipcRenderer.invoke('input:devices'),
    ...
    onDeviceConnected: (cb: (device: ControllerDevice) => void) => {
      const handler = (_: Electron.IpcRendererEvent, dev: ControllerDevice) => cb(dev)
      ipcRenderer.on('input:device-connected', handler)
      return () => ipcRenderer.removeListener('input:device-connected', handler)
    },
    onDeviceDisconnected: (cb: (id: string) => void) => {
      ...
      return () => ipcRenderer.removeListener('input:device-disconnected', handler)
    }
  },

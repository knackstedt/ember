...
export interface PluginApi {
  ...
  addChipFilter(tab: string, filter: { id: string; label: string; predicate: (item: unknown) => boolean }): void
...
export function createPluginApi(manifest: PluginManifest): PluginApi {
  return {
    ...
    addChipFilter(tab, filter) {
      console.log(`[plugin:${manifest.id}] Add chip filter on tab "${tab}": ${filter.id}`)
    },
    onIpc(channel, handler) {
      const { ipcMain } = require('electron')
      ipcMain.handle(`plugin:${manifest.id}:${channel}`, (_e: unknown, ...args: unknown[]) =>
        handler(...args)
      )
    },
    log(message) {

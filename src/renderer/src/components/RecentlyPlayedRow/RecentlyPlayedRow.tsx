...
const RecentCard: React.FC<{ item: RecentlyPlayedItem; onLaunch: (id: string) => void }> = ({
  item,
  onLaunch
}) => {
    ...
    >
      <div className="relative w-full aspect-[2/3] overflow-hidden">
        {showPlaceholder ? (
          ...
          >
            <span className="text-xl font-bold text-white/40">{initials(item.title)}</span>
          </div>
        ) : (
          <img
            src={item.coverUrl}
...
export const RecentlyPlayedRow: React.FC<RecentlyPlayedRowProps> = ({ items, onLaunch }) => {

...
export const XboxController: React.FC<XboxControllerProps> = ({ highlightCode, learnCode }) => {
  ...
  return (
    ...
    >
      ...
      {Object.entries(BUTTON_POSITIONS).map(([code, b]) => {
        const active = accentCode === code
        const baseColor = active ? (b.color ?? 'var(--color-accent)') : 'var(--color-surface)'
        const borderColor = active ? (b.color ?? 'var(--color-accent)') : 'var(--color-border)'
        return (
          <g key={code}>
            <circle

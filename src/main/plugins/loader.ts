...
import { registeredPlugins, RegisteredPlugin, createPluginApi, PluginModule } from './api'
...
async function loadPluginFromDir(pluginDir: string): Promise<RegisteredPlugin | null> {
  ...
  try {
    const fn = new Function('require', 'module', 'exports', '__dirname', '__filename', buildResult.code)
    const mod = { exports: {} as PluginModule }
    fn(require, mod, mod.exports, pluginDir, entryPath)
    pluginModule = (mod.exports as unknown as { default?: PluginModule }).default ?? mod.exports
  } catch (err) {
    const resolved = await resolveSourceMappedStack(err as Error, buildResult.map)
    errorMsg = resolved
...
export async function reloadPlugins(): Promise<RegisteredPlugin[]> {
  for (const plugin of registeredPlugins) {
    if (plugin.module.deactivate) {
      try { plugin.module.deactivate() } catch { /* */ }
    }
  }
